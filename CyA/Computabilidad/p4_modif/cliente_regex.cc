...
void EncuentraVariables(std::vector<Variables>& vector_variables, const std::string& linea, const int& contador) {
  std::regex expression{"\\bint\\s*[a-zA-Z]+\\s*=?\\s*\\d*\\s*;\\s*$|\\bdouble\\s*[a-zA-Z]+\\s*=?\\s*\\d*.?\\d*\\s*;\\s*$"};
  std::smatch matches;
...
// void EncuentraInclude(std::vector<std::string>& vector_includes, const std::string& linea, const int& contador) {
//   std::regex expresion{""};
//   std::smatch coincidencias;
//   if (std::regex_search(linea, coincidencias, expresion)) {
//     vector_includes.push_back(coincidencias.str());
//     vector_includes.push_back(std::to_string(contador));
//   }
// }
