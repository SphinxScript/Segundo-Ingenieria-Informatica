...
class HeapSort {
 public:
  HeapSort(StaticSeq<Key>& secuencia, unsigned size, unsigned& asignaciones);
...
template <class Key>
HeapSort<Key>::HeapSort(StaticSeq<Key>& secuencia, unsigned size, unsigned& asignaciones) : size_(size), secuencia_(secuencia), asignaciones_(asignaciones) {
  Sort();
}
