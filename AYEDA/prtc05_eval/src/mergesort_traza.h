...
class MergeSort {
 public:
  MergeSort(StaticSeq<Key>& secuencia, unsigned size, unsigned& asignaciones);
...
template <class Key>
MergeSort<Key>::MergeSort(StaticSeq<Key>& secuencia, unsigned size, unsigned& asignaciones) :  size_(size), secuencia_(secuencia), asignaciones_(asignaciones) {
  if (size_ == 0) return;
  ...
  std::cout << "Número de asignaciones realizadas: " << asignaciones_ << std::endl;
}
